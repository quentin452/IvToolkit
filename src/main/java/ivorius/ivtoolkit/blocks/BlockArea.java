...
public class BlockArea implements Iterable<BlockCoord> {
    ...
    public static BlockArea areaFromSize(BlockCoord coord, int[] size) {
        if (size[0] <= 0 || size[1] <= 0 || size[2] <= 0) throw new IllegalArgumentException();

        return new BlockArea(
            coord,
            new BlockCoord(coord.x + size[0] - 1, coord.y + size[1] - 1, coord.z + size[2] - 1));
    }
    ...
    public boolean contains(BlockCoord coord) {
        ...
        return coord.x >= lower.x && coord.y >= lower.y
            && coord.z >= lower.z
            && coord.x <= higher.x
            && coord.y <= higher.y
            && coord.z <= higher.z;
