...
public class IvWorldData {
    ...
    public IvWorldData(World world, BlockArea blockArea, boolean captureEntities) {
        ...
        for (BlockCoord worldCoord : blockArea) {
            ...
            blockCollection
                .setMetadata(dataCoord, (byte) world.getBlockMetadata(worldCoord.x, worldCoord.y, worldCoord.z));
        ...
        if (captureEntities) {
            entities = world
                .getEntitiesWithinAABBExcludingEntity(null, blockArea.asAxisAlignedBB(), new EntitySelectorSaveable());
        } else {
    ...
    public static void moveTileEntityForGeneration(TileEntity tileEntity, BlockCoord coord) {
        if (tileEntity instanceof Movable) ((Movable) tileEntity).move(coord.x, coord.y, coord.z);
        else {
    ...
    public static void transformTileEntityPosForGeneration(TileEntity tileEntity, AxisAlignedTransform2D transform,
        int[] size) {
        if (tileEntity instanceof Transformable)
            ((Transformable) tileEntity).transform(transform.getRotation(), transform.isMirrorX(), size);
        else setTileEntityPosForGeneration(tileEntity, transform.apply(new BlockCoord(tileEntity), size));
    }

    public static void moveEntityForGeneration(Entity entity, BlockCoord coord) {
        if (entity instanceof Movable) ((Movable) entity).move(coord.x, coord.y, coord.z);
        else {
            ...
            if (entity instanceof EntityCreature) {
                ...
                EntityCreatureAccessor.setHomePosition(
                    entityCreature,
                    homePosition.posX + coord.x,
                    homePosition.posY + coord.y,
                    homePosition.posZ + coord.z);
            }
        }
    }
    ...
    public static void transformEntityPosForGeneration(Entity entity, AxisAlignedTransform2D transform, int[] size) {
        if (entity instanceof Transformable)
            ...
            if (entity instanceof EntityHanging) {
                EntityHanging entityHanging = (EntityHanging) entity;
                BlockCoord hangingCoord = new BlockCoord(
                    entityHanging.field_146063_b,
                    entityHanging.field_146064_c,
                    entityHanging.field_146062_d);
            ...
            if (entity instanceof EntityCreature) {
                ...
                EntityCreatureAccessor
                    .setHomePosition(entityCreature, newHomePosition.posX, newHomePosition.posY, newHomePosition.posZ);
            }
        }
    }
    ...
    public static void injectTEBlockFixTags(NBTTagCompound compound, String tileEntityID, NBTTagList list,
        String... keys) {
        if (tileEntityID.equals(compound.getString("id"))) {
            for (String key : keys) if (hasPrimitive(compound, key)) addBlockTag(compound.getInteger(key), list, key);
        }
    }
    ...
    public static void applyIDFixTags(NBTTagCompound compound, MCRegistry registry) {
        if (compound.hasKey(ID_FIX_TAG_KEY)) {
            ...
            for (int i = 0; i < list.tagCount(); i++) applyIDFixTag(compound, registry, list.getCompoundTagAt(i));
        }
    }
    ...
    public static void applyIDFixTag(NBTTagCompound compound, MCRegistry registry, NBTTagCompound fixTag) {
        ...
        switch (type) {
            case "item": {
                ...
                if (item != null) compound.setInteger(dest, Item.getIdFromItem(item));
                else IvToolkitCoreContainer.logger
                    .warn("Failed to fix item tag from structure with ID '" + stringID + "'");
            ...
            case "block": {
                ...
                if (block != null) compound.setInteger(dest, Block.getIdFromBlock(block));
                else IvToolkitCoreContainer.logger
                    .warn("Failed to fix block tag from structure with ID '" + stringID + "'");
                break;
            }
            default:
