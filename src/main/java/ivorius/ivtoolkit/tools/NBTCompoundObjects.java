...
public class NBTCompoundObjects {
    ...
    public static <K extends NBTCompoundObject> K readFrom(NBTTagCompound compound, String key,
    ...
    public static <T extends NBTCompoundObject> List<T> readListFrom(NBTTagCompound compound, String key,
        Class<T> tClass) {
    ...
    public static <T extends NBTCompoundObject> List<T> readList(NBTTagList list, Class<T> tClass) {
        ...
        for (int i = 0; i < list.tagCount(); i++) rList.add(read(list.getCompoundTagAt(i), tClass));

        return rList;
    }

    public static <K extends NBTCompoundObject, V extends NBTCompoundObject> Map<K, V> readMapFrom(
        NBTTagCompound compound, String key, Class<? extends K> keyClass, Class<? extends V> valueClass) {
        return readMap(compound.getTagList(key, Constants.NBT.TAG_COMPOUND), keyClass, valueClass);
    }

    public static <K extends NBTCompoundObject, V extends NBTCompoundObject> Map<K, V> readMap(NBTTagList nbt,
    ...
    public static <K extends NBTCompoundObject, V extends NBTCompoundObject> void writeMapTo(NBTTagCompound compound,
