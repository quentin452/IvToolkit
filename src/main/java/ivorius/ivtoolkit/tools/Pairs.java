...
public class Pairs {
        ...
        return Sets.newHashSet(
            Iterables.concat(
                StreamSupport.stream(set.spliterator(), false)
                    .map(Pairs.<T>leftFunction()::apply)
                    .collect(Collectors.toList()),
                StreamSupport.stream(set.spliterator(), false)
                    .map(Pairs.<T>rightFunction()::apply)
                    .collect(Collectors.toList())));
    }
    ...
    public static <L, R> Iterable<Pair<L, R>> pairLeft(final L left, Iterable<R> right) {
        return StreamSupport.stream(right.spliterator(), false)
            .map(input -> Pair.of(left, input))
            .collect(Collectors.toList());
    }
