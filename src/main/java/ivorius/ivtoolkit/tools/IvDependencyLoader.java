...
public abstract class IvDependencyLoader {
    ...
    private String loadResource(String k, List<String> loading) throws LoadException, RecursionException {
        ...
        if (loading.contains(k)) throw new RecursionException();
        loading.add(k);

        res = loadResourceRaw(k);
        int startIndex, endIndex;

        while ((startIndex = res.indexOf(importStart)) >= 0 && (endIndex = res.indexOf(importEnd)) >= 0) {
    ...
    public static class RecursionException extends Exception {
        ...
        public RecursionException(String message, Throwable cause, boolean enableSuppression,
