...
public class ModelRenderer {
    ...
    private static void renderNodePart(Tessellator tessellator, NodePart nodePart) {
        ...
        if (material.has(TextureAttribute.Diffuse)) {
            ...
            if (textureCoordAttr == null) uvs = guessUVs(meshPart.primitiveType, texture, meshPart.numVertices);
        ...
        } else GL11.glColor3f(rgb[0], rgb[1], rgb[2]);
        ...
        for (VertexAttribute attribute : vertexAttributes) {
            if (attribute.usage == VertexAttributes.Usage.BoneWeight) boneWeightAttributes.add(attribute);
        }
        ...
        for (int i = meshPart.indexOffset; i < meshPart.numVertices + meshPart.indexOffset; i++) {
            ...
            if (texture != null) {
                ...
                } else if (uvs != null) tessellator.setTextureUV(uvs[i * 2], uvs[i * 2 + 1]);
            }
            ...
            } else tessellator.addVertex(vertexX, vertexY, vertexZ);
        }
        tessellator.draw();

        if (texture == null) GL11.glEnable(GL11.GL_TEXTURE_2D);

        if (blend != null) GL11.glDisable(GL11.GL_BLEND);
    }

    public static void buildMatrix(Matrix4f dst, Collection<VertexAttribute> boneWeightAttributes,
        FloatBuffer floatBuffer, int vertexIndex, Matrix4f[] bones) {
        dst.setZero();
    ...
    public static float[] guessUVs(int primitiveType, Texture texture, int length) {
        ...
        for (int i = 0; i < uvs.length / 2; i++) // Go through all combinations... Works for quads, the rest at least
                                                 // gets some variety
        {
            switch (i % 4) {
                case 0:
