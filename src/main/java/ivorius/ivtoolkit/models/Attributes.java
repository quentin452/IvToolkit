...
public class Attributes implements Iterable<Attribute>, Comparator<Attribute> {
    ...
    public final Attribute get(final long type) {
        if (has(type)) for (Attribute attribute : attributes) if (attribute.type == type) return attribute;
        return null;
    }
    ...
    public final List<Attribute> get(final List<Attribute> out, final long type) {
        for (Attribute attribute : attributes) if ((attribute.type & type) != 0) out.add(attribute);
        return out;
    }
    ...
    /**
     * Add a attribute to this material. If the material already contains an attribute of the same type it is
     * overwritten.
     */
    ...
    /**
     * Removes the attribute from the material, i.e.: material.remove(BlendingAttribute.ID); Can also be used to remove
     * multiple
    ...
    /**
     * @return True if this collection has the specified attribute, i.e. attributes.has(ColorAttribute.Diffuse); Or when
     *         multiple
     *         attribute types are specified, true if this collection has all specified attributes, i.e.
     *         attributes.has(out,
    ...
    protected int indexOf(final long type) {
        if (has(type)) for (int i = 0; i < attributes.size(); i++) if (attributes.get(i).type == type) return i;
        return -1;
    }
    ...
    public final boolean same(final Attributes other, boolean compareValues) {
        ...
        for (int i = 0; i < attributes.size(); i++) if (!attributes.get(i)
            .equals(other.attributes.get(i))) return false;
        return true;
    }
    ...
    public int hashCode() {
        ...
        int m = 1;
        for (Attribute attribute : attributes) result += mask * attribute.hashCode() * (m = (m * 7) & 0xFFFF);
        return result;
    }
