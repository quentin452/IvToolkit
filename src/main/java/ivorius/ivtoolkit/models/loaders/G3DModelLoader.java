...
public class G3DModelLoader implements ModelLoader {
    ...
    public static Model modelFromRawModel(G3DModel g3DModel, TextureProvider textureProvider, Logger logger) {
        if (g3DModel.version == null) g3DModel.version = new short[] { VERSION_HI, VERSION_LO };
    ...
    private static void loadNodes(Model model, List<G3DNode> g3DNodes, Map<String, Node> nodeMap) {
        ...
        for (Material material : model.materials) materials.put(material.id, material);

        Map<String, MeshPart> meshParts = new HashMap<>();
        for (MeshPart meshPart : model.meshParts) meshParts.put(meshPart.id, meshPart);

        for (G3DNode g3DNode : g3DNodes) model.nodes.add(convertNodeWithoutParts(g3DNode, nodeMap));
    ...
    public static Node convertNodeWithoutParts(G3DNode g3DNode, Map<String, Node> nodeMap) {
        ...
        if (g3DNode.translation != null) node.translation.set(g3DNode.translation);

        if (g3DNode.rotation != null) node.rotation.set(g3DNode.rotation);

        if (g3DNode.scale != null) node.scale.set(g3DNode.scale);

        if (g3DNode.children != null) for (G3DNode g3dChild : g3DNode.children) {
    ...
    public static void convertNodeParts(Node node, G3DNode g3DNode, Map<String, Node> nodes,
        Map<String, Material> materials, Map<String, MeshPart> meshParts) {
        if (g3DNode.parts != null) for (G3DNodePart g3DNodePart : g3DNode.parts)
            node.parts.add(convertNodePart(g3DNodePart, nodes, materials, meshParts));
    }

    public static NodePart convertNodePart(G3DNodePart g3DNodePart, Map<String, Node> nodes,
        Map<String, Material> materials, Map<String, MeshPart> meshParts) {
        ...
        if (nodePart.material == null) nodePart.material = new Material();
    ...
    private static void loadMaterials(Model model, List<G3DMaterial> g3DMaterials, TextureProvider textureProvider,
        Logger logger) {
    ...
    public static Material convertMaterial(G3DMaterial g3DMaterial, TextureProvider textureProvider, Logger logger) {
        ...
        if (g3DMaterial.ambient != null) material.set(new ColorAttribute(ColorAttribute.Ambient, g3DMaterial.ambient));
        if (g3DMaterial.diffuse != null) material.set(new ColorAttribute(ColorAttribute.Diffuse, g3DMaterial.diffuse));
        ...
        if (g3DMaterial.textures != null) {
            for (G3DTexture tex : g3DMaterial.textures) {
                ...
                if ((tex.uvScaling != null && (tex.uvScaling.getX() != 1.0f || tex.uvScaling.getY() != 1.0f))
                    || (tex.uvTranslation != null
                        && (tex.uvTranslation.getX() != 0.0f || tex.uvTranslation.getY() != 0.0f))) {
                    ...
                    texture = new TextureSub(
                        texture,
                        trans.getX(),
                        trans.getY(),
                        trans.getX() + scale.getX(),
                        trans.getY() + scale.getY());
                }
                ...
                switch (tex.type) {
                    ...
                    case "NORMAL":
                        ...
                        break;
                    default:
                        if (logger != null) logger.warn("Unknown texture type: '" + tex.type + "'");
                }
            }
        }
    ...
    private static void loadAnimations(Model model, List<G3DAnimation> animations, Map<String, Node> nodes) {
        for (G3DAnimation animation : animations) model.animations.add(convertAnimation(animation, nodes));
    }
    ...
    public static Animation convertAnimation(G3DAnimation g3DAnimation, Map<String, Node> nodes) {
        ...
        for (NodeAnimation nodeAnimation : animation.nodeAnimations) {
            for (NodeKeyframe nodeKeyframe : nodeAnimation.keyframes) {
                if (nodeKeyframe.keytime > highestKeytime) highestKeytime = nodeKeyframe.keytime;
            }
        }
    ...
    public static NodeAnimation convertNodeAnimation(G3DNodeAnimation g3DNodeAnimation, Map<String, Node> nodes) {
        ...
        for (G3DKeyframe keyframe : g3DNodeAnimation.keyframes) nodeAnimation.addKeyframe(convertKeyframe(keyframe));
    ...
    public static VertexAttribute[] parseVertexAttributes(String[] attributes) {
        ...
        for (String attribute : attributes) {
            if (attribute.equals("POSITION")) vertexAttributes.add(VertexAttribute.Position());
            else if (attribute.equals("NORMAL")) vertexAttributes.add(VertexAttribute.Normal());
            else if (attribute.equals("COLOR")) vertexAttributes.add(VertexAttribute.ColorUnpacked());
            else if (attribute.equals("COLORPACKED")) vertexAttributes.add(VertexAttribute.ColorPacked());
            else if (attribute.equals("TANGENT")) vertexAttributes.add(VertexAttribute.Tangent());
            else if (attribute.equals("BINORMAL")) vertexAttributes.add(VertexAttribute.Binormal());
            else if (attribute.startsWith("TEXCOORD")) vertexAttributes.add(VertexAttribute.TexCoords(unit++));
