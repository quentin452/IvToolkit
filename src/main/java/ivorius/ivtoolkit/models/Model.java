...
public class Model {
     ...
     * Then each
     * {@link Node#calculateGlobalTransform()} is calculated, based on the parent's world transform and the local
     * transform of each
    ...
    public void calculateTransforms() {
        for (Node node : nodes) node.calculateTransforms(true);
        for (Node node : nodes) node.calculateBoneTransforms(true);
    }
}
