 ...
 * {@link ivorius.ivtoolkit.models.Mesh} instances to define
 * its vertex structure. Vertex attributes have an order. The order is specified by the order they are added to this
 * class.
 *
 * @author mzechner
 */
public final class VertexAttributes implements Iterable<VertexAttribute> {
    ...
    public VertexAttribute findByUsageAndUnit(int usage, int unit) {
        int len = size();
        for (int i = 0; i < len; i++) if (get(i).usage == usage && get(i).unit == unit) return get(i);
        return null;
    }
    ...
    private int calculateOffsets() {
        ...
        for (VertexAttribute attribute : attributes) {
            ...
            if (attribute.usage == Usage.ColorPacked) count += 4;
    ...
    /**
     * Calculates a mask based on the contained {@link VertexAttribute} instances. The mask is a bit-wise or of each
     * attributes
