...
public class ReferenceCounter<V> {
    ...
    public boolean release(V object, int releaseCount) {
        ...
        if (newCount < 0) throw new RuntimeException("Attempting to release a freed object!");
        else if (newCount == 0) {
            freeObjects.add(object);
            map.remove(object);
        } else map.put(object, newCount);

        return newCount > 0;
    }

    public Set<V> deallocateAllFreeObjects() {
        if (freeObjects.size() == 0) return Collections.emptySet();
