...
public class MazeGenerator {
    ...
    public static <T> void generateStartPathsForEnclosedMaze(Maze<T> maze, Iterable<MazePath> startPoints,
        Iterable<MazeRoom> blockedRooms, AxisAlignedTransform2D transform, T path, T room, T blockedPath) {
        int[] mazeSizeInRooms = new int[maze.dimensions.length];
        for (int i = 0; i < mazeSizeInRooms.length; i++) mazeSizeInRooms[i] = (maze.dimensions[i] - 1) / 2;

        maze.allPaths()
            .stream()
            .filter(maze::isPathPointingOutside)
            .forEach(exitPath -> maze.set(blockedPath, exitPath));
    ...
    public static <T> MazePath randomEmptyPathInMaze(Random rand, Maze<T> maze,
        Collection<Integer> applicableDimensions) {
        ...
        if (paths.size() > 0) return paths.get(rand.nextInt(paths.size()));
    ...
    public static MazePath randomPathInMaze(Random rand, Maze maze, int... distanceFromOutside) {
        ...
        pathCoord[usedDimension] = rand.nextInt(availablePaths) * 2
            + (maze.dimensions[usedDimension] / 2 + 1 - availablePaths) / 2;
