...
public class MazeGeneratorClassic {
    ...
    public static void generateStartPathsForEnclosedMaze(Maze<Byte> maze, Iterable<MazePath> startPoints,
        Iterable<MazeRoom> blockedRooms, AxisAlignedTransform2D transform) {
    ...
    public static void generatePaths(Random rand, Maze<Byte> maze, int[] pathWeights, MazeRoom startPoint) {
        for (int i = 0; i < maze.dimensions.length; i++) if (maze.dimensions[i] < 3) return;
        ...
        while (true) {
            ...
            for (MazePath neighbor : MazeCoordinates.getNeighborPaths(position)) {
                if (maze.isNull(maze.get(neighbor.getDestinationRoom()))) {
                    for (int n = 0; n < pathWeights[neighbor.getPathDimension()]; n++) validRoomNeighbors.add(neighbor);
                }
            }
            ...
            for (MazePath neighbor : neighbors) if (maze.isInvalid(maze.get(neighbor))) maze.set(WALL, neighbor);

            position = destRoom;
        }

        for (int i = 0; i < maze.blocks.length; i++) if (maze.isInvalidOrNull(maze.blocks[i])) maze.blocks[i] = WALL; // Should
                                                                                                                      // not
                                                                                                                      // happen.
                                                                                                                      // Potentially.
    }
