...
public class MazePath implements MazeCoordinate {
    ...
    public static MazePath fromConnection(int[] source, int[] dest) {
        for (int i = 0; i < source.length; i++) {
            if (source[i] != dest[i]) return new MazePath(i, source[i] < dest[i] ? source : dest);
        }
    ...
    public MazeRoom getDestinationRoom() {
        return destRoomCache != null ? destRoomCache : (destRoomCache = sourceRoom.addInDimension(pathDimension, 1));
    }
