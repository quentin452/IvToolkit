...
public class Maze<T> {
    ...
    public Maze(Class<T> tClass, T invalidValue, T nullValue, int... dimensions) {
        ...
        if (nullValue != null) Arrays.fill(blocks, nullValue);
    }
    ...
    public boolean contains(MazeCoordinate coordinate) {
        ...
        for (int i = 0; i < position.length; i++) {
            if (position[i] < 0 || position[i] >= this.dimensions[i]) return false;
        }
    ...
    public void set(T val, MazeCoordinate coordinates) {
        if (contains(coordinates)) blocks[getArrayPosition(coordinates.getMazeCoordinates())] = val;
    }
    ...
    public void replace(T condition, T val, MazeCoordinate coordinates) {
        if (contains(coordinates)) {
            ...
            if (Objects.equals(blocks[arrayPosition], condition)) blocks[arrayPosition] = val;
        }
    }
    ...
    public int[] getRoomSize(MazeCoordinate coordinate, int[] pathLengths, int[] roomWidths) {
        ...
        for (int i = 0; i < returnSize.length; i++) returnSize[i] = isRoomPath[i] ? pathLengths[i] : roomWidths[i];
    ...
    public static boolean isCoordValidRoom(MazeCoordinate coordinate) {
        ...
        for (boolean b : isRoomPath) if (b) return false;
    ...
    public boolean isPathPointingOutside(MazeCoordinate coordinate) {
        ...
        for (int dim = 0; dim < dimensions.length; dim++)
            if (mazePosition[dim] == 0 || mazePosition[dim] == dimensions[dim] - 1) return true;
    ...
    public List<MazeRoom> allRooms() {
        if (cachedRooms == null) {
            ...
            for (int i = 0; i < blocks.length; i++) {
                ...
                if (room != null) coordinates.add(room);
            }
    ...
    public List<MazePath> allPaths() {
        if (cachedPaths == null) {
            ...
            for (int i = 0; i < blocks.length; i++) {
                ...
                if (path != null) coordinates.add(path);
            }
