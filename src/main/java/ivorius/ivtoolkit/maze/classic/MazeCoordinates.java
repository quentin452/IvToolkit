...
public class MazeCoordinates {
    ...
    public static MazeRoom rotatedRoom(MazeRoom room, AxisAlignedTransform2D transform, int[] size) {
        ...
        BlockCoord transformedRoom = transform
            .apply(new BlockCoord(roomPosition[0], roomPosition[1], roomPosition[2]), size);
    ...
    public static MazePath rotatedPath(MazePath path, AxisAlignedTransform2D transform, int[] size) {
        int[] sourceCoords = path.getSourceRoom()
            .getCoordinates();
        int[] destCoords = path.getDestinationRoom()
            .getCoordinates();
        BlockCoord transformedSource = transform
            .apply(new BlockCoord(sourceCoords[0], sourceCoords[1], sourceCoords[2]), size);
        BlockCoord transformedDest = transform.apply(new BlockCoord(destCoords[0], destCoords[1], destCoords[2]), size);

        return MazePath.fromConnection(
            new MazeRoom(transformedSource.x, transformedSource.y, transformedSource.z),
            new MazeRoom(transformedDest.x, transformedDest.y, transformedDest.z));
    }
    ...
    private static MazeRoom[] constructNeighborRooms(int dimensions) {
        ...
        for (int i = 0; i < neighborPaths.length; i++) neighbors[i] = neighborPaths[i].getDestinationRoom();
    ...
    public static MazeRoom[] getNeighborRooms(int dimensions) {
        ...
        return cachedNeighborRoomsBlueprints.get(dimensions)
            .clone();
    }
    ...
    public static MazePath[] getNeighborPaths(MazeRoom mazeRoom) {
        ...
        for (int i = 0; i < blueprints.length; i++) neighbors[i] = blueprints[i].add(mazeRoom);
    ...
    public static MazeRoom coordToRoom(MazeCoordinate coordinate) {
        if (Maze.isCoordValidRoom(coordinate)) {
            ...
            for (int dim = 0; dim < roomCoord.length; dim++) roomCoord[dim] = (roomCoord[dim] - 1) / 2;
    ...
    public static MazePath coordToPath(MazeCoordinate coordinate, int pathDim) {
        ...
        for (int dim = 0; dim < roomCoord.length; dim++) {
            if (roomCoord[dim] == 0) goesUp = false;
            else roomCoord[dim] = (roomCoord[dim] - 1) / 2;
        }
