...
public class MazeComponents {
    ...
    public static <M extends MazeComponent<C>, C> Function<M, Stream<ShiftedMazeComponent<M, C>>> shiftAllFunction(
        final MazePassage connection, final C connector, final ConnectionStrategy<C> strategy) {
        return component -> component.exits()
            .entrySet()
            .stream()
            .map(entry -> {
                MazeRoom dist = entry.getKey()
                    .inverseDistance(connection);
