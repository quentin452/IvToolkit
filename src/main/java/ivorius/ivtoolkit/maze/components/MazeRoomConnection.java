...
public class MazeRoomConnection extends Pair<MazeRoom, MazeRoom> {
    ...
    public MazeRoomConnection(MazeRoom left, MazeRoom right) {
        ...
        if (left.getDimensions() != right.getDimensions()) throw new IllegalArgumentException();
    }

    private static boolean lower(MazeRoom left, MazeRoom right) {
        for (int i = 0; i < left.getDimensions(); i++)
            if (left.getCoordinate(i) != right.getCoordinate(i)) return left.getCoordinate(i) < right.getCoordinate(i);
    ...
    public String toString() {
        return "MazeRoomConnection{" + "left=" + left + ", right=" + right + '}';
    }
}
