...
public class GridQuadCache<T> implements Iterable<GridQuadCache.CachedQuadLevel<T>> {
    ...
    public static class CoordGrid extends TIntArrayList {
        ...
        private static boolean isFree(boolean[][] mask, int lX, int hX, int y) {
            for (int tX = lX; tX <= hX; tX++) if (!mask[tX][y]) return false;
        ...
        public QuadCollection computeMesh(int minX, int minY, int maxX, int maxY) {
            ...
            for (int c = 0; c < coordCount(); c++) mask[x(c)][y(c)] = true;

            for (int x = minX; x < maxX; x++) for (int y = minY; y < maxY; y++) {
                if (mask[x][y]) {
                    // Expand X
                    int lX = x, hX = x, lY = y, hY = y;
                    while (lX > minX && mask[lX - 1][y]) lX--;
                    while (hX < maxX - 1 && mask[hX + 1][y]) hX++;

                    // Expand Y
                    while (lY > minY && isFree(mask, lX, hX, lY - 1)) lY--;
                    while (hY < maxY - 1 && isFree(mask, lX, hX, hY + 1)) hY++;

                    // Fill mask
                    for (int tX = lX; tX <= hX; tX++) for (int tY = lY; tY <= hY; tY++) mask[tX][tY] = false;
