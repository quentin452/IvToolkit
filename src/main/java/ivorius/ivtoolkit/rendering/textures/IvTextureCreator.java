...
public class IvTextureCreator {
    ...
    public static BufferedImage applyEffect(BufferedImage texture, ImageEffect effect) {
        ...
        {
            BufferedImage notIndexed = new BufferedImage(
                texture.getWidth(),
                texture.getHeight(),
                BufferedImage.TYPE_INT_ARGB);
            notIndexed.getGraphics()
                .drawImage(texture, 0, 0, null);
            texture = notIndexed;
        }
        ...
        for (int x = 0; x < texture.getWidth(); x++) for (int y = 0; y < texture.getHeight(); y++) {
